...
<body>
    <header>
        <div class="header-items">
            ...
            <nav>
                <ul>
                    <li><a class="active" href="#">Проживающие</a></li>
            ...
            <div class="main-content">
                ...
                <div class="newdates" style="display: none;">
                    <div class="newdates-item">
                        <label for="name">Имя проживающего:</label>
                    </div>
                    <div class="newdates-item">
                    ...
                    <div class="newdates-item">
                            <?php
                                $stmt = $db->prepare("SELECT id, room_number FROM rooms");
                                $stmt->execute();
                                $Rooms = $stmt->fetchAll(PDO::FETCH_ASSOC);
                                echo    '<select name="room_number">';
                                print('<option disabled selected>выберите комнату</option>');
                                foreach ($Rooms as $room) {
                                    printf('<option value="%d">№ %s</option>', $room['id'], $room['room_number']);
                                }
                echo            '</select>';
                            ?>
                        </select>
            ...
            <?php
                ...
                foreach ($values as $value) {
                    ...
                    echo            '<select'; if(empty($_COOKIE['edit']) || ($_COOKIE['edit'] != $value['id'])) print(" disabled ");
                                        else print(" "); echo 'name="room_number'.$value['id'].'">';
                                    foreach ($Rooms as $room) {
                                        if ($room['id'] == $value['room_number']) {
                                            printf('<option selected value="%d">№ %s</option>', $room['id'], $room['room_number']);
                                        } else {
                                            printf('<option value="%d">№ %s</option>', $room['id'], $room['room_number']);
                                        }
                                    }
